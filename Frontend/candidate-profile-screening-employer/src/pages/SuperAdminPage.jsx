...
import { MapPin, Users, Plus, UserPlus, CheckCircle, XCircle, Edit, Trash2 } from 'lucide-react';
...
const SuperAdminPage = () => {
  ...
  const handleOnboardSubmit = async (e) => {
    ...
    try {
      ...
      showToastMessage(
        `${onboardData.role} onboarded successfully with email: ${onboardData.email}`,
        'success'
      );
      setOnboardData({ email: '', role: '' });
      setShowOnboardModal(false);
    } catch (error) {
      showToastMessage(error.message || 'Onboarding failed', 'error');
    } finally {
  ...
  return (
    <div className="min-h-screen p-6 bg-gray-50">
      ...
      {/* Onboard User Modal - Only for SuperAdmin */}
      {user?.role === 'SUPERADMIN' && showOnboardModal && (
